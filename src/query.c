...
int checkRefuse(char * areaName)
{
    ...
    {
        w_log(LL_ERR,
              "Can't open newAreaRefuseFile \"%s\" : %d\n",
              af_robot->newAreaRefuseFile,
              strerror(errno));
        return 0;
    }
...
void del_tok(char ** ac, char * tok)
{
    ...
    if(q)
    {
        ...
        if(*p)
        {
            memmove(q, p + 1, strlen(p + 1) + 1); /*  begin or middle */
        }
        else
        {
            if(q > *ac)
...
char * makeAreaParam(s_link * creatingLink, s_link_robot * r, char * c_area, char * msgbDir)
{
    ...
    {
        msgbDir =
            creatingLink->areafix.baseDir ? creatingLink->areafix.baseDir : af_config->msgBaseDir;
    }
    ...
    {
        /*  passthrough */
        xscatprintf(&buff, "EchoArea %s%s%s passthrough", quote_areaname, d_area, quote_areaname);
...
e_BadmailReasons autoCreate(char * c_area, char * descr, hs_addr pktOrigAddr, ps_addr forwardAddr)
{
    ...
    {
        w_log(LL_WARN,
              "Can't create %s %s : refused by %s newAreaRefuseFile\n",
              af_robot->strA,
              c_area,
              af_robot->name);
    ...
    fileName =
        r->autoCreateFile ? r->autoCreateFile : (af_cfgFile ? af_cfgFile : getConfigFileName());
    f = fopen(fileName, "a+b");

    if(f == NULL)
    {
        w_log(LL_ERR,
              "%s::autoCreate(): cannot open config file \"%s\", link %s, OS error: \"%s\", please check configuration (and run tparser!)",
              __FILE__,
              fileName,
              aka2str5d(creatingLink->hisAka),
              strerror(errno));
...
//    fprintf(stderr, "autocreate: cannot open af_config file\n");
    ...
    else if(af_app->module == M_HTICK)
    {
        ...
        if(af_config->areasFileNameCase == eUpper)
        {
            strUpper(fileechoFileName);
        }
        else
        {
            strLower(fileechoFileName);
        }

        if(bDir == NULL)
        {
            bDir =
                (creatingLink->filefix.baseDir) ? creatingLink->filefix.baseDir : af_config->
                fileAreaBaseDir;
        }
    ...
    if(!isLinkOfArea(creatingLink, area))
    {
        ...
        if(af_config->createAddUplink)
        {
            ...
            if(area)
            {
                area->downlinks[area->downlinkCount - 1]->defLink = 1;
            }
        }
    }
    ...
    /*  or not existing links */
    {
        for(i = 1; i < areaNode->linksCount; i++)
        {
            if((isAreaLink(areaNode->downlinks[i],
                           area) == -1) &&
               (getLinkFromAddr(af_config,
                                areaNode->downlinks[i])) &&
    ...
    if(af_app->module == M_HPT)
    {
        carbonNames2Addr(af_config);
    }

    w_log(LL_AUTOCREATE, "%s %s autocreated by %s", af_robot->strA, c_area, hisaddr);

    if(hook_onAutoCreate)
    {
        (*hook_onAutoCreate)(c_area, descr, pktOrigAddr, forwardAddr);
    }
    ...
    {
        if(pauseArea(ACT_PAUSE, NULL, area))
        {
            sendAreafixMessages();
        }
    }
...
s_query_areas * af_CheckAreaInQuery(char * areatag,
                                    ps_addr uplink,
                                    ps_addr dwlink,
                                    e_query_action act)
{
    ...
    switch(act)
    {
        ...
        case FINDFREQ: /* Find area with "freq" or "idle" state in query list (query-in-progress
                          area) */
        ...
        case ADDIDLE: /* Create idle forward request for area into query list */
            ...
            else
            {
                ...
                w_log(LL_AREAFIX,
                      "%s: make request idle for area: %s",
                      af_robot->name,
                      areaNode->name);
                tmpNode = areaNode;
            }
        ...
        case DELIDLE: /* Remove idle forward request to area from query list */
            ...
            if(bFind && stricmp(tmpNode->type, czIdleArea) == 0)
            {
                ...
                tmpNode->type[0]      = '\0';
                w_log(LL_AREAFIX,
                      "%s: idle request for %s removed from queue file",
                      af_robot->name,
                      tmpNode->name);
            }
...
char * af_Req2Idle(char * areatag, char * report, hs_addr linkAddr)
{
    ...
    while(tmpNode->next)
    {
        ...
        if((areaNode->name) &&
           (stricmp(areaNode->type, czFreqArea) == 0) && (patimat(areaNode->name, areatag) == 1))
        {
            i = 1;
            ...
            if(i < areaNode->linksCount)
            {
                ...
                if(areaNode->linksCount == 1)
                {
                    ...
                    if(UPlink)
                    {
                        forwardRequestToLink(areaNode->name, UPlink, NULL, 1);
                    }
                }

                xscatprintf(&report, " %s %s  request canceled\r", areaNode->name,
                            repeat_char(49 - strlen(areaNode->name), '.'));
                w_log(LL_AREAFIX,
...
void af_QueueReport(void)
{
    ...
    while(tmpNode->next)
    {
        ...
        if(stricmp(tmpNode->type, czFreqArea) == 0)
        {
            ...
            if(strcmp(tmpNode->type, czFreqArea) == 0)
            {
                ...
                xscatprintf(&report, rmask, tmpNode->name, tmpNode->type, link1, link2, "request");
                continue;
            }

            if(af_report_changes)
            {
                continue;                    /* report changes in queue file only */
            }
            ...
            xscatprintf(&report, rmask, tmpNode->name, type, link1, link2, state);
        }
        ...
        if(stricmp(tmpNode->type, czKillArea) == 0)
        {
            if(strcmp(tmpNode->type, czKillArea) == 0)
            {
                queryAreasHead->nFlag = 1;
                strUpper(tmpNode->type);
                xscatprintf(&report, rmask, tmpNode->name, tmpNode->type, link1, "", "timeout");
                continue;
            }
            ...
            xscatprintf(&report, rmask, tmpNode->name, type, link1, "", state);
        }
    ...
    xscatprintf(&header, rmask, ucStrA, "Act", "From", "By", "Details");
    xscatprintf(&header, "%s\r", repeat_char(79, '-'));
    xstrcat(&header, report);
    report = header;

    if(af_config->ReportTo)
    {
        if(stricmp(af_config->ReportTo, "netmail") == 0)
        {
            netmail = 1;
        }
        else if(getNetMailArea(af_config, af_config->ReportTo) != NULL)
        {
            netmail = 1;
        }
    }
    ...
    msg = makeMessage(&(af_config->addr[0]),
                      &(af_config->addr[0]),
                      af_robot->fromName ? af_robot->fromName : af_versionStr,
                      netmail ? (af_config->sysop ? af_config->sysop : "Sysop") : "All",
                      "Requests report",
                      netmail,
                      af_robot->reportsAttr);
    msg->text =
        createKludges(af_config, netmail ? NULL : af_config->ReportTo, &(af_config->addr[0]),
                      &(af_config->addr[0]), af_versionStr);
    ...
    while(tmpNode->next)
    {
        ...
        if(stricmp(tmpNode->type, czFreqArea) == 0)
        {
            ...
            else
            {
                ...
                w_log(LL_AREAFIX,
                      "%s: request for %s is going to be killed",
                      af_robot->name,
                      tmpNode->name);
                    ...
                    for(j = 0; j < af_config->linkCount; j++)
                    {
                           ...
                           dwlink->sendNotifyMessages)
                        {
                            if(tmpmsg[j] == NULL)
                            {
                                ...
                                rf =
                                    r->reportsFlags ? r->reportsFlags : af_robot->reportsFlags;
                                ...
                                xstrcat(&tmpmsg[j]->text,
                                        "\r Your requests for the following areas were forwarded to uplinks,\r");
                                xscatprintf(&tmpmsg[j]->text,
                                            " but no messages were received at least in %u days. Your requests\r",
                                            af_robot->forwardRequestTimeout);
        ...
        if(stricmp(tmpNode->type, czKillArea) == 0)
        {
            ...
            tmpNode->type[0]      = '\0';
            w_log(LL_AREAFIX,
                  "%s: request for %s removed from queue file",
                  af_robot->name,
                  tmpNode->name);
            continue;
        }
        ...
        if(stricmp(tmpNode->type, czIdleArea) == 0)
        {
            ...
            if(delarea && !mandatoryal)
            {
                do_delete(dwlink, delarea);
            }

            /* unsubscribe at uplink */
            if(dwlink && !(delarea && mandatoryal))
            {
                forwardRequestToLink(tmpNode->name, dwlink, NULL, 2);
            }
        }
    }
    ...
    while((line = readLine(queryFile)) != NULL)
    {
        ...
        if(token != NULL)
        {
            ...
            while(token != NULL)
            {
                areaNode->linksCount++;
                areaNode->downlinks =
                    (*call_srealloc)(areaNode->downlinks, sizeof(hs_addr) * areaNode->linksCount);
                memset(&(areaNode->downlinks[areaNode->linksCount - 1]), 0, sizeof(hs_addr));
                parseFtnAddrZS(token, &(areaNode->downlinks[areaNode->linksCount - 1]));
                token = strtok(NULL, seps);
            }
        }
...
void af_CloseQuery(void)
{
    ...
    while(tmpNode)
    {
        if(writeChanges && tmpNode->type[0] != '\0')
        {
            ...
            sprintf(p,
                    "%s %d-%02d-%02d@%02d:%02d\t%d-%02d-%02d@%02d:%02d",
                    tmpNode->type,
                    t1.tm_year + 1900,
                    t1.tm_mon + 1,
                    t1.tm_mday,
...
int checkAreaLink(s_area * area, hs_addr aka, int type)
{
    ...
    if(writeAccess == 0 && area->paused)
    {
        writeAccess = BM_AREA_IS_PAUSED;
    }
